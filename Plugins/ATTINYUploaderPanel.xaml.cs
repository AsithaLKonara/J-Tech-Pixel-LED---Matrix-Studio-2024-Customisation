...
{
    public partial class ATTINYUploaderPanel : UserControl
    {
        public ATTINYUploaderPanel()
        {
            InitializeComponent();
            DataContext = new ATTINYUploaderViewModel(UploadSnackbar);
        }
    }

    public class ATTINYUploaderViewModel : INotifyPropertyChanged
    {
        ...
        public string Programmer { get => programmer; set { programmer = value; OnPropertyChanged(); } }
        private string chipType = "attiny85";
        public string ChipType { get => chipType; set { chipType = value; OnPropertyChanged(); } }
        private string log;
        ...
        public ATTINYUploaderViewModel(Snackbar snackbar)
        {
        ...
        private async void Upload()
        {
            ...
            {
                _snackbarQueue?.Enqueue("Please enter a programmer type (e.g., usbasp).");
                return;
            }
            if (string.IsNullOrWhiteSpace(ChipType))
            {
                _snackbarQueue?.Enqueue("Please enter a chip type (e.g., attiny85).");
                return;
            }
            IsUploading = true;
